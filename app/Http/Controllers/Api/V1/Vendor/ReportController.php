    ...
    private function order_report_formatter($data): array
    {
        ...
        foreach ($data as $item) {
               $storage[] = [
                ...
                'total_item_amount' => (int)  ($item->order_amount- $item->additional_charge - $item->dm_tips-$item->total_tax_amount-$item->delivery_charge+$item->coupon_discount_amount + $item->restaurant_discount_amount),
                'item_discount' => (int)  $item->details->sum('discount_on_food'),
                'coupon_discount' => (int)  $item->coupon_discount_amount,
                'discounted_amount' => (int)  ($item->coupon_discount_amount + $item->restaurant_discount_amount),
                'tax' => (int)  $item->total_tax_amount,
                'delivery_charge' => (int)  $item->delivery_charge,
                'additional_charge' => (int)  $item->additional_charge,
    ...
    private function transaction_report_formatter($data): array
    {
        ...
        foreach ($data as $item) {
            ...
            $storage[] = [
                ...
                'total_item_amount' =>(int)  ($item->order['order_amount'] - $item->additional_charge  -  $item->order['dm_tips']-$item->order['delivery_charge'] - $item['tax'] + $item->order['coupon_discount_amount'] + $item->order['restaurant_discount_amount']),
                'item_discount' =>(int)  $item->order->details->sum('discount_on_food'),
                'coupon_discount' => (int) $item->order['coupon_discount_amount'],
                'discounted_amount' => (int) ($item->order['coupon_discount_amount'] + $item->order['restaurant_discount_amount']),
                'vat' => (int) $item->tax,
                'delivery_charge' =>(int)  ($item->delivery_charge + $item->delivery_fee_comission),
                'order_amount' => (int) $item->order_amount,
                'admin_discount' =>(int)  $item->admin_expense,
                'restaurant_discount' => (int) $item->discount_amount_by_restaurant,
                'admin_commission' =>(int) ( $item->admin_commission  + $item->admin_expense  - $item->additional_charge ),
                'additional_charge' => (int) $item->additional_charge,
                'commission_on_delivery_charge' => (int) $item->delivery_fee_comission,
                'admin_net_income' =>(int)  ($item->admin_commission + $item->delivery_fee_comission) ,
                'restaurant_net_income' => (int) ($item->restaurant_amount - $item->tax),
    ...
    private function food_report_formatter($data): array
    {
        ...
        foreach ($data as $item) {
            $storage[] = [
                ...
                'total_discount_given' => (int)  $item->orders_sum_discount_on_food,
                'average_sale_value' => (int) ($item->orders_count>0? ($item->orders_sum_price-$item->orders_sum_discount_on_food)/$item->orders_count:0 ),
